...
const getLocalH5PPath = (title) => {
  if (!title) return null;
  const t = title.toLowerCase().trim();
  if (t.includes("chung cư mini")) return "CHAYCHUNGCUMINI";
  if (t.includes("bình nóng lạnh")) return "CHAYBINHNONGLANH";
  // Video 3 "Cháy chung cư" loads from remote embed since local version has missing video files.
  // if (t.includes('cháy chung cư') && !t.includes('mini')) return 'CHAYCHUNGCU';
  if (t.includes("ấm đun nước")) return "CHAYAMDUNNUOC";
  return null;
};
...
function Classroom({
  ...
  useEffect(() => {
    ...
    const onMessage = (e) => {
      ...
      const verbDisp =
        stmt && stmt.verb && stmt.verb.display
          ? Object.values(stmt.verb.display).join(" ")
          : "";
      ...
      if (result && result.score && onScoreRef.current && video?.id) {
        ...
        if (!(max > 0) && Number.isFinite(Number(sc.scaled))) {
          max = 100;
          raw = Math.round(Number(sc.scaled) * 100);
        }
        if (Number.isFinite(raw) && max > 0)
          onScoreRef.current(video.id, raw, max);
      }
  ...
  // Khởi tạo Local H5P Player nếu là bài học cục bộ
  useEffect(() => {
    const localPath = getLocalH5PPath(video?.title);
    if (!localPath) return;

    let active = true;
    let handleXApi;

    // Đợi container DOM render xong
    const timer = setTimeout(() => {
      const container = document.getElementById("h5p-container");
      if (!container || !active) return;

      new H5P(container, {
        h5pJsonPath: `/h5p/${localPath}`,
        ...
        .then(() => {
          ...
          if (window.H5P && window.H5P.externalDispatcher) {
            handleXApi = (event) => {
              ...
              const verb =
                (statement.verb.display &&
                  Object.values(statement.verb.display)[0]) ||
                statement.verb.id.split("/").pop();
              const result = statement.result;

              // Xử lý hoàn thành bài học
              if (verb === "completed" || verb === "passed") {
                if (onProgressRef.current && video?.id) {
                  onProgressRef.current(video.id, 100);
                }
                if (onCompleteRef.current && video?.id) {
                  onCompleteRef.current(video.id);
                }
              }
              ...
              if (result && result.score && onScoreRef.current && video?.id) {
                ...
                if (!(max > 0) && Number.isFinite(Number(sc.scaled))) {
                  max = 100;
                  raw = Math.round(Number(sc.scaled) * 105); // scaling
                }
                if (Number.isFinite(raw) && max > 0) {
                  onScoreRef.current(video.id, raw, max);
                }
            ...
            window.H5P.externalDispatcher.on("xAPI", handleXApi);
          }
        })
        .catch((err) => {
          console.error("Local H5P Init Error:", err);
        });
    }, 100);
    ...
    return () => {
      active = false;
      clearTimeout(timer);
      if (window.H5P && window.H5P.externalDispatcher && handleXApi) {
        try {
          window.H5P.externalDispatcher.off("xAPI", handleXApi);
        } catch (e) {
          console.warn("Failed to unsubscribe xAPI listener:", e);
        }
      }
    };
  }, [video?.id]);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div
          className="classroom-lesson-header"
        ...
        >
          ...
          >
            <span
              className={`video-category-tag ${video?.categoryKey || "co-ban"}`}
              style={{ padding: "4px 10px", fontSize: "0.7rem" }}
            >
              {video?.category || "Cơ bản"}
            </span>
            <span
        ...
        >
          {showQuiz ? (
            ...
            <div
              className="classroom-quiz-panel"
              style={{
                width: "100%",
                height: "100%",
            ...
            >
              ...
              <h2
                className="classroom-quiz-title"
                style={{
                  fontSize: "1.45rem",
                  fontWeight: "800",
                  marginBottom: "24px",
              ...
              >
                <label
                  className="classroom-quiz-option"
                  style={{
                    display: "flex",
                    alignItems: "center",
                    gap: "12px",
          ...
          ) : getLocalH5PPath(video.title) ? (
            <div
              id="h5p-container"
              key={video.id || video._id}
              className="h5p-standalone-container"
              style={{
                width: "100%",
                height: "100%",
                minHeight: "480px",
            ...
            /h5p\.com|youtube\.com|youtu\.be|vimeo\.com|\/embed/i.test(
              video.videoUrl,
            ) &&
            !/\.(mp4|webm|ogg|mov|m3u8)(\?|$)|\/video\/upload\//i.test(
              video.videoUrl,
            ) ? (
            <iframe
              src={video.videoUrl}
        ...
        {/* Navbar Tabs mockup */}
        <div
          className="classroom-section-tabs"
          style={{
            display: "flex",
        ...
        <div id="classroom-tabs-content">
          {activeTab === "desc" && (
            <div>
              ...
              <div
                className="classroom-meta-strip"
                style={{
                  display: "flex",
                  flexWrap: "wrap",
                  gap: "12px",
              ...
              <div
                className="classroom-feature-cards"
              ...
              >
                ...
                >
                  ...
                  <div>
                    ...
                    >
                      Hoàn thành bài kiểm tra để ghi nhận tiến độ học tập
                    </p>
                  </div>
                </div>
          ...
          {activeTab === "desc" && (
            ...
            >
              <div
                className="classroom-discussion-header"
                style={{
                  display: "flex",
              ...
              <div
                className="classroom-comment-form-row"
                style={{ display: "flex", gap: "16px", marginBottom: "30px" }}
              >
                <div
