...
const PCCC_QUIZ_POOL = [
  {
    question:
      "Khi xảy ra cháy nhà cao tầng, bạn nên sử dụng phương tiện di chuyển nào để thoát hiểm?",
    optionA: "Thang máy tiện lợi nhanh chóng",
    optionB: "Thang bộ thoát hiểm kiên cố cách nhiệt",
    correct: "B",
    correctMessage:
      "🟢 Chính xác! Thang bộ thoát hiểm là lối thoát khẩn cấp an toàn nhất, được thiết kế chịu lực và ngăn khói độc tốt. Tránh dùng thang máy vì hỏa hoạn có thể gây cắt điện đột ngột và mắc kẹt cabin độc!",
    incorrectMessage:
      "🔴 Chưa chính xác! Trong trường hợp hỏa hoạn tuyệt đối KHÔNG sử dụng thang máy vì hệ thống điện có thể bị ngắt đột ngột gây kẹt cabin và khói độc dễ tràn vào giếng thang máy gây ngạt thở.",
  },
  {
    question:
      "Nếu bị mắc kẹt trong phòng đầy khói độc của đám cháy, tư thế di chuyển nào là an toàn nhất cho bạn?",
    optionA: "Cúi thật thấp người hoặc bò sát mặt đất",
    optionB: "Chạy thẳng người thật nhanh ra ngoài",
    correct: "A",
    correctMessage:
      "🟢 Chính xác! Khói độc và khí nóng nhẹ hơn không khí nên sẽ bay lơ lửng ở phần sát trần nhà. Di chuyển sát mặt đất sẽ giúp bạn thở được luồng dưỡng khí mát lành hơn ở phía dưới!",
    incorrectMessage:
      "🔴 Chưa chính xác! Chạy thẳng đứng sẽ làm đầu bạn đi thẳng vào vùng khói độc siêu đậm đặc nguy hiểm nhất, làm suy hô hấp và ngất xỉu chỉ trong vài giây.",
  },
  {
    question:
      "Khi thoát hiểm xuyên qua vùng khói lửa dày đặc, bạn nên bảo vệ đường hô hấp bằng cách nào?",
    optionA: "Dùng tay khô che miệng mũi",
    optionB: "Dùng khăn hoặc vải tẩm ướt bịt kín miệng mũi",
    correct: "B",
    correctMessage:
      "🟢 Chính xác! Khăn ướt đóng vai trò như chiếc khẩu trang sinh học tinh lọc bớt bụi mịn, tàn tro nguy hiểm và hạ nhiệt cho luồng không khí nóng bạn hít thở vào phổi!",
    incorrectMessage:
      "🔴 Chưa chính xác! Bàn tay khô không thể lọc được các hạt khói mịn độc hại và tàn tro nguy kịch, bạn vẫn hít trực tiếp khí độc vào cơ thể.",
  },
  {
    question:
      "Khi có đám cháy nhỏ phát sinh do chập thiết bị điện trong nhà, việc đầu tiên bạn bắt buộc phải làm là gì?",
    optionA: "Dội một xô nước lạnh thật lớn để dập tắt lửa",
    optionB: "Tìm và ngắt ngay cầu dao điện/Aptomat chính của nhà",
    correct: "B",
    correctMessage:
      "🟢 Chính xác! Việc cắt nguồn điện là bước ưu tiên cấp thiết hàng đầu để chặn đứng dòng sinh nhiệt tiếp diễn và đảm bảo tuyệt đối an toàn không bị điện giật xung quanh!",
    incorrectMessage:
      "🔴 Chưa chính xác! Dội nước vào đám cháy điện cực kỳ nguy hiểm, nước dẫn điện sẽ gây hiện tượng nổ tung thiết bị và truyền giương điện xung quanh gây giật chết người.",
  },
  {
    question:
      "Trước khi vội vã mở bất cứ cánh cửa phòng nào để tháo chạy ra hành lang trong đám cháy, bạn nên kiểm tra thế nào?",
    optionA: "Dùng mu bàn tay chạm thử vào bề mặt tay nắm cửa",
    optionB: "Đẩy thật mạnh mở toang cửa để nhìn bên ngoài",
    correct: "A",
    correctMessage:
      "🟢 Chính xác! Chạm mu bàn tay vào tay nắm cửa giúp cảm nhận nhiệt phòng bên. Nếu tay nắm nóng bỏng, chứng tỏ lửa lớn hoành hoành đang bao vây bên ngoài, tuyệt đối không được mở!",
    incorrectMessage:
      "🔴 Chưa chính xác! Đẩy toang cửa không kiểm tra sẽ lập tức tạo luồng gió hút khí nóng oxy, làm lửa tạt thẳng vào cơ thể gây bỏng sâu nghiêm trọng.",
  },
];
...
function Classroom({
  ...
  const fetchComments = async () => {
    ...
    try {
      const response = await axios.get(
        `${API_BASE_URL}/api/auth/comments/${video.id}`,
      );
  ...
  const handleSendComment = async (e) => {
    ...
    try {
      ...
      const msg =
        err.response?.data?.message || "Có lỗi xảy ra khi gửi bình luận!";
      showToast(msg, "error");
    } finally {
  ...
  const handleQuizSubmit = (e) => {
    ...
    if (isCorrect) {
      ...
      setTimeout(() => {
        ...
        showToast(
          "Chúc mừng bạn đã hoàn thành bài học! Hãy để lại cảm nghĩ và đánh giá của bạn nhé.",
          "success",
        );
  ...
  return (
    <div
      className="classroom-grid-layout"
      style={{ width: "100%", marginTop: "10px" }}
    >
      {/* LEFT COLUMN: Video Player & description & comments review */}
      <div
        className="classroom-main-col"
      ...
      >
        ...
        <div
          className={`video-player-box ${video?.videoUrl?.includes("h5p.com") ? "h5p-active" : "normal-active"}`}
        >
          {showQuiz ? (
            ...
            <div
              style={{
                ...
                backgroundImage:
                  "radial-gradient(ellipse at top, #fee2e2 0%, #f8fafc 70%)",
                padding: "40px",
                display: "flex",
            ...
            >
              ...
              >
                <span style={{ fontSize: "1.1rem" }}>🚨</span> Tình huống khẩn
                cấp
              </div>
                ...
                {!quizSubmitted ? (
                  ...
                  <div
                    style={{
                      ...
                      borderRadius: "10px",
                      backgroundColor:
                        selectedQuizOption === currentQuiz?.correct
                          ? "#ecfdf5"
                          : "#fef2f2",
                      color:
                        selectedQuizOption === currentQuiz?.correct
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        const randomIndex = Math.floor(
                          Math.random() * PCCC_QUIZ_POOL.length,
                        );
                        setCurrentQuiz(PCCC_QUIZ_POOL[randomIndex]);
                      }}
                      style={{
            ...
            <video
              ...
              onEnded={() => {
                ...
                showToast(
                  "Bạn đã xem hết video! Bài học đã được ghi nhận hoàn thành.",
                  "success",
                );
                if (onComplete && video?.id) {
                  onComplete(video.id);
        ...
        >
          <div>
            <div style={{ display: "flex", gap: "8px", marginBottom: "10px" }}>
              <span
                className="video-category-tag co-ban"
                style={{ padding: "4px 10px", fontSize: "0.7rem" }}
              >
                Cơ bản
              </span>
              <span
                className="video-category-tag ky-thuat"
                style={{ padding: "4px 10px", fontSize: "0.7rem" }}
              >
                Kỹ thuật
              </span>
        ...
        <div id="classroom-tabs-content">
              ...
              >
                ...
                >
                  ...
                  <div>
                    ...
                    >
                      Hoàn thành đạt 80% để nhận chứng chỉ
                    </p>
                  </div>
                </div>
                ...
                >
                  ...
                  <div>
                    ...
                    >
                      Hệ thống tự động lưu các mốc quan trọng
                    </p>
                  </div>
                </div>
              </div>
              ...
              >
                <div>
                  <h4 style={{ fontSize: "0.88rem", fontWeight: "700" }}>
                    📄 Cam kết & Giáo trình kỹ thuật PCCC.pdf
                  </h4>
                  <p style={{ fontSize: "0.75rem", color: "#94a3b8" }}>
                    Dung lượng: 4.8 MB • Định dạng PDF
                  </p>
                </div>
                <button
                  onClick={() =>
                    showToast(
                      "Đang tải tài liệu học tập của bài học!",
                      "success",
                    )
                  }
                  style={{
          ...
          >
            ...
            >
              ...
              >
                Sắp xếp theo:{" "}
                <span style={{ color: "#1e293b", cursor: "pointer" }}>
                  Mới nhất ▾
                </span>
              </div>
            </div>
            ...
            <div style={{ display: "flex", gap: "16px", marginBottom: "30px" }}>
              ...
              >
                <span
                  className="profile-widget-fallback"
            ...
            >
              {loadingComments ? (
                <p style={{ color: "#64748b", fontSize: "0.85rem" }}>
                  Đang nạp các bình luận và đánh giá...
                </p>
              ) : comments.length === 0 ? (
                <p style={{ color: "#64748b", fontSize: "0.85rem" }}>
                    ...
                    >
                      ...
                      >
                        ...
                        <span
                          ...
                          onClick={() =>
                            showToast(
                              "Tính năng trả hồi bài viết sẽ được hỗ trợ sớm.",
                              "success",
                            )
                          }
                        >
